...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		try{
			String[] splited = sourceText.split(" ");
			...
			for(int i = 0; i < splited.length; i++){
				...
				for(ListNode node:wordList){
					if(node.getWord().toString().equals(starter)){
						node.addNextWord(splited[i]);
						break;
					}
					count+=1;
				}	
	...
	public String generateText(int numWords) {
		...
		try{
			...
			String output="";
			String temp=null;
			//System.out.println(wordList.get(1).getRandomNextWord(rnGenerator));
			...
			while(output.split(" ").length < numWords){
				...
				for(ListNode node:wordList){
					if(node.getWord().equals(starter)){
						temp=node.getRandomNextWord(rnGenerator);
						output=output+temp+" ";
						starter=temp;
						Found=true;
	                    break;
	...
	public void retrain(String sourceText)
	{
		...
		try{
			...
			String[] splited = sourceText.split("\\s+");
			//for(String s:splited){
			//	System.out.println(s);
			//}
